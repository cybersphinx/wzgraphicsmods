...
/// Lightmap image
static GLubyte* lightmap_pixmap;
...
#define q(i,j,center) ((x*ySectors+y)*(sectorSize+1)*(sectorSize+1)*2 + ((i)*(sectorSize+1)+(j))*2+(center))
	...
	lightmap_pixmap = (GLubyte* )calloc( 1, lightmapSize * lightmapSize * 3 * sizeof(GLubyte));
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, lightmapSize, lightmapSize, 0, GL_RGB, GL_UNSIGNED_BYTE, lightmap_pixmap);
...
void shutdownTerrain(void)
{
	...
	free(lightmap_pixmap);
	lightmap_pixmap = NULL;
	...
	if (gameTime > lightmapNextUpdate)
	{
		...
		for (i = 0; i < lightmapSize; i++)
		{
			for (j = 0; j < lightmapSize; j++)
			{
				...
				lightmap_pixmap[(i * lightmapSize + j) * 3 + 0] = colour.byte.r;
				lightmap_pixmap[(i * lightmapSize + j) * 3 + 1] = colour.byte.g;
				lightmap_pixmap[(i * lightmapSize + j) * 3 + 2] = colour.byte.b;
				...
				if (!rendStates.fogEnabled)
				{
					...
					if (darken < 1)
					{
						lightmap_pixmap[(i * lightmapSize + j) * 3 + 0] *= darken;
						lightmap_pixmap[(i * lightmapSize + j) * 3 + 1] *= darken;
						lightmap_pixmap[(i * lightmapSize + j) * 3 + 2] *= darken;
					}
				}
			}
		}
		...
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, lightmapSize, lightmapSize, GL_RGB, GL_UNSIGNED_BYTE, lightmap_pixmap);
	}
