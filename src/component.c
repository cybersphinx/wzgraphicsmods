...
static void displayCompObj(DROID *psDroid, BOOL bButton)
{
		...
		(!cyborgDroid(psDroid))) && (!bButton))
	{
		...
		if ( psDroid->sMove.Status != MOVEINACTIVE )
		{
			...
			if ( psJet != NULL )
			{
				pie_Draw3DShape(psJet, getModularScaledGraphicsTime(psJet->animInterval, psJet->numFrames), colour, brightness, specular, pie_ADDITIVE, 200);
			}
		}
	}
			...
			case DROID_CYBORG_REPAIR:
			{
				...
				if(psShape)
				{
					    ...
					    psShape->nconnectors && psDroid->action == DACTION_DROIDREPAIR)
					{
						...
						pie_Draw3DShape(psShape, getModularScaledGraphicsTime(psShape->animInterval, psShape->numFrames), 0, brightness, WZCOL_BLACK, pie_ADDITIVE, 140);
							/* Dither off software */
