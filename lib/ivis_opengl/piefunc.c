...
void pie_DrawSkyboxOverlay(float scale, int u, int v, int w, int h)
{
	const float r = 1.0f; // just because it is shorter than 1.0f
	...
	glBegin(GL_QUAD_STRIP);
		...
		glTexCoord2f(u + w * 0, v + h);	glVertex3f(-r, 0, r); // bottom left
		glTexCoord2f(u + w * 0, v);		glVertex3f(-r, r, r); // top left
		glTexCoord2f(u + w * 0.5, v + h);	glVertex3f( r, 0, r); // bottom right
		glTexCoord2f(u + w * 0.5, v); 	glVertex3f( r, r, r); // top right

		// Right
		glTexCoord2f(u + w * 1.0, v + h);	glVertex3f( r, 0,-r); // bottom r
		glTexCoord2f(u + w * 1.0, v); 	glVertex3f( r, r,-r); // top r

		// Back
		glTexCoord2f(u + w * 1.5, v + h);	glVertex3f(-r, 0, -r); // bottom right
		glTexCoord2f(u + w * 1.5, v); 	glVertex3f(-r, r, -r); // top right

		// Left
		glTexCoord2f(u + w * 2, v + h);	glVertex3f(-r, 0, r); // bottom r
		glTexCoord2f(u + w * 2, v); 	glVertex3f(-r, r, r); // top r
	glEnd();
